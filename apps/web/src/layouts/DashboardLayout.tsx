...
const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navItems = [
    { name: 'Monitoring Grid', path: '/', icon: Grid },
    { name: 'Check Definitions', path: '/checks', icon: Terminal },
    { name: 'Live Results', path: '/results', icon: Activity },
    { name: 'Incident Correlation', path: '/correlation', icon: Zap },
    { name: 'Performance Heatmap', path: '/heatmap', icon: BarChart4 },
    { name: 'Alert History', path: '/alerts', icon: AlertCircle },
    { name: 'Agent Regions', path: '/agents', icon: Globe },
    ...
    <div className="flex h-screen bg-slate-950 text-slate-200 overflow-hidden font-sans">
      ...
      <aside className={`bg-slate-900 border-r border-slate-800 transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-20'} flex flex-col`}>
        <div className="p-6 flex items-center gap-4 border-b border-slate-800">
          ...
          {sidebarOpen && <span className="font-bold text-xl tracking-tight bg-gradient-to-r from-white to-rose-400 bg-clip-text text-transparent">SYNTH-GRID</span>}
        </div>
          ...
          <div className="flex items-center gap-3 p-2 bg-slate-800/50 rounded-xl">
            ...
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate">Ops Architect</p>
                <p className="text-xs text-slate-500 truncate">Synthetic Admin</p>
              </div>
            )}
          </div>
        </div>
        ...
        <header className="h-16 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-6 z-10">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="relative max-w-md w-full">
              ...
              <input 
                type="text" 
                placeholder="Search checks, results, or alerts..." 
